 ...
 * @author dzajac
 */
public class LoyaltyPromotionAgent implements ContextAgent
{

    public LoyaltyPromotionAgent(CRMProcessor processor)
    {
        super();
    ...
    public void execute(Context ctx) throws AgentException
    {
        ...
        try
        {
            ...
            try
            {
                ...
                // the Raw Price Plan Version has all the services, not only the subscribed ones
                PricePlanVersion ppv = sub.getRawPricePlanVersion(ctx);
                PricePlan pricePlan = ppv.getPricePlan(ctx);
                ...
                {
                    ratePlanFilter = getRatePlanFilter(ctx, bundleDefinition.getType(), pricePlan);
                }
                else if (bundleDefinition.getBundleCategoryIds().size()>0)
                {
                    ...
                    while (iter.hasNext())
                    {
                        ...
                        filter = getRatePlanFilter(ctx, association.getType(), pricePlan);
        ...
        {
            new MinorLogMsg(ctx, "Failed to process ER 881 because of Exception " + e.getMessage(), e).log(ctx);
    ...
    protected static final String PM_MODULE = LoyaltyPromotionAgent.class.getName();
    ...
    protected static final int REQUIRED_LENGTH = 7;
}
