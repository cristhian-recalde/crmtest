 ...
 */
public class BirthdayPlanValidator extends ContextAwareSupport implements Validator
{
    public BirthdayPlanValidator(final Context ctx)
    {
        setContext(ctx);
    }
    ...
    public void validate(final Context ctx, final Object obj) throws IllegalStateException
    {
        ...
        validateBirthdayPlanLicense(ctx, exceptions);

        if (plan.getName() == null || plan.getName().equals(""))
        {
            exceptions.thrown(new IllegalPropertyArgumentException(
                    BirthdayPlanXInfo.NAME,
                    "Empty names are not allowed"));
        }

        
        if (plan.getBpServiceType().equals(CallingGroupServiceTypeEnum.ALL) || plan.getBpServiceType().equals(CallingGroupServiceTypeEnum.VOICE))
        {
            validateVoiceFields(plan, exceptions);
        }
        
        if (plan.getBpServiceType().equals(CallingGroupServiceTypeEnum.ALL) || plan.getBpServiceType().equals(CallingGroupServiceTypeEnum.SMS))
        {
            validateSmsFields(plan, exceptions);
        }

        exceptions.throwAll();
    }

    private void validateSmsFields(BirthdayPlan plan, CompoundIllegalStateException exception)
    {
    ...
    private void validateVoiceFields(BirthdayPlan plan, CompoundIllegalStateException exception)
    {
    ...
    private void validateBirthdayPlanLicense(final Context ctx, final CompoundIllegalStateException exceptions)
    {
        if (!LicensingSupportHelper.get(ctx).isLicensed(ctx, LicenseConstants.FNF_BIRTHDAYPLAN_LICENSE))
        {
            final String msg = "Birthday Plan License is not Enabled";
            exceptions.thrown(new IllegalStateException(msg));
        }
    }
