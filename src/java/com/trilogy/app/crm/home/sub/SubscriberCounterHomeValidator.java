...
/**
 * Since the subscriber Counter check only has to be done on a create, this HomeValidator will return the
 * appropriate create and store validators for use by the ValidatingHome.
 *
 * @author bdhavalshankh
 * @since 9.5.1
 */
public class SubscriberCounterHomeValidator implements HomeValidator
{
    ...
    public static HomeValidator instance()
    {
        if( instance_ == null )
        {
            instance_ = new SubscriberCounterHomeValidator();
        }
        return instance_;
    }
    ...
    {
        return SubscriberCounterValidator.instance();
    }
