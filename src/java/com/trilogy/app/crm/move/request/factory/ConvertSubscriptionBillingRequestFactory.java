 ...
 * @since 8.1
 */
class ConvertSubscriptionBillingRequestFactory
{

    static SubscriptionMoveRequest getInstance(Context ctx, Subscriber subscription)
    {
        ConvertSubscriptionBillingTypeRequest request = new ConvertSubscriptionBillingTypeRequest();

        if (subscription != null)
        {
            request.setSubscriberType(subscription.getSubscriberType());
        }
        
        if (request == null)
        {
            request = new ConvertSubscriptionBillingTypeRequest();
        }
