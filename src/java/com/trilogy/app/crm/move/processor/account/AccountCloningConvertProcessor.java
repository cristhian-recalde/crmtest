 ...
 * @since 9.5.1
 */
public class AccountCloningConvertProcessor<AMR extends AccountMoveRequest> extends CopyMoveProcessor<AMR>
{
    public AccountCloningConvertProcessor(MoveProcessor<AMR> delegate)
    {
        super(delegate, NullCopyMoveStrategy.instance());
    }


    public AccountCloningConvertProcessor(MoveProcessor<AMR> delegate, CopyMoveStrategy<AMR> strategy)
    {
    ...
    {   
        ...
        if (account != null)
        {
            try
            {
            	...
            	{
            		new DebugLogMsg(this, "Cloning account " + account.getBAN() + " in order to convert it while retaining the old copy...", null).log(ctx);
            	}
            ...
            if (newAccount != null)
            {
                ...
                newAccount.setNextSubscriberId(newAccount.DEFAULT_NEXTSUBSCRIBERID);
