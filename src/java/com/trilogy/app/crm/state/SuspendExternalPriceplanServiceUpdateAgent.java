 ...
 * @since 9.6
 */
public class SuspendExternalPriceplanServiceUpdateAgent implements ServiceStateUpdateAgent
{
    @Override
    public void update(Context ctx, Subscriber subscriber, Service service) throws HomeException
    {
        
        int result = SuspensionSupport.suspendGenericService(ctx, subscriber, service, true, true);
        if(result == SuspensionSupport.EXTERNAL_FAIL)
        {
            String msg = " Failure result code returned: " +result+ ". Can not update Subscriber."+ 
                         " Error occurred while executing suspend commands for external" +
                         " priceplan service.  ";
