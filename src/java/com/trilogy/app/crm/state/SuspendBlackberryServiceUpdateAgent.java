 ...
 *
 */
public class SuspendBlackberryServiceUpdateAgent extends AbstractBlackberryServiceUpdateAgent
{
            ...
            Subscriber subscriber) throws ServiceBlackberryException
    {
        try
        {
            ...
            bbService.suspend(ctx, subscriber.getSpid(), services, attr);
        }
    ...
    {
        return ServiceSupport.getServiceSuspendConfigs(subscriber, service);
    }
    ...
    if (SystemSupport.needsHlr(ctx))
        ...
        {
            LogSupport.debug(ctx, this, "Calling HLR to Suspend Blackberry service " +service.toString());
        }
	    try{
	    	long response = ServiceProvisioningGatewaySupport.execute(ctx, CommandID.SERVICE_SUSPEND,
