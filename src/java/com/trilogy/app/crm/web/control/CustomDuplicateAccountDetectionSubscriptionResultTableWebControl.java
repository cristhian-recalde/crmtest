...
import com.trilogy.app.crm.bean.duplicatedetection.DuplicateAccountDetectionSubscriptionResultIdentitySupport;
import com.trilogy.app.crm.bean.duplicatedetection.DuplicateAccountDetectionSubscriptionResultTableWebControl;
import com.trilogy.app.crm.bean.duplicatedetection.DuplicateAccountDetectionSubscriptionResultXInfo;
 ...
 * @since 2010-12-23
 */
public class CustomDuplicateAccountDetectionSubscriptionResultTableWebControl
    extends DuplicateAccountDetectionSubscriptionResultTableWebControl
{
	@Override
	public void toWeb(Context ctx, PrintWriter out, String name, Object obj)
	{
		...
		renderer.Table(ctx, out, mmgr.get(
		    "DuplicateAccountDetectionSubscriptionResult.Label",
		    DuplicateAccountDetectionSubscriptionResultXInfo.Label));
		...
		ViewModeEnum subscriptionType_mode =
		    getMode(subCtx,
		        "DuplicateAccountDetectionSubscriptionResult.subscriptionType");
		ViewModeEnum phoneNumber_mode =
		    getMode(subCtx,
		        "DuplicateAccountDetectionSubscriptionResult.phoneNumber");
		ViewModeEnum state_mode =
		    getMode(subCtx, "DuplicateAccountDetectionSubscriptionResult.state");
		if (subscriptionType_mode != ViewModeEnum.NONE)
		{
			out.println("<th TITLE=\""
			    + mmgr
			        .get(
			            "DuplicateAccountDetectionSubscriptionResult.subscriptionType.Label",
			            DuplicateAccountDetectionSubscriptionResultXInfo.SUBSCRIPTION_TYPE.getLabel(ctx))
			    + "\" >");
			out.println(
			    mmgr.get(
			        "DuplicateAccountDetectionSubscriptionResult.subscriptionType.ColumnLabel",
			        DuplicateAccountDetectionSubscriptionResultXInfo.SUBSCRIPTION_TYPE.getColumnLabel(ctx)));

			out.println("</th>");

		}

		if (phoneNumber_mode != ViewModeEnum.NONE)
		{
			out.println("<th TITLE=\""
			    + mmgr
			        .get(
			            "DuplicateAccountDetectionSubscriptionResult.phoneNumber.Label",
			            DuplicateAccountDetectionSubscriptionResultXInfo.PHONE_NUMBER.getLabel(ctx))
			    + "\" >");
			out.println(mmgr
			    .get(
			        "DuplicateAccountDetectionSubscriptionResult.phoneNumber.ColumnLabel",
			        DuplicateAccountDetectionSubscriptionResultXInfo.PHONE_NUMBER.getColumnLabel(ctx)));

			out.println("</th>");

		}

		if (state_mode != ViewModeEnum.NONE)
		{

			out.println("<th TITLE=\""
			    + mmgr
			        .get(
			            "DuplicateAccountDetectionSubscriptionResult.state.Label",
			            DuplicateAccountDetectionSubscriptionResultXInfo.STATE.getLabel(ctx))
			    + "\" >");
			out.println(mmgr
			    .get(
			        "DuplicateAccountDetectionSubscriptionResult.state.ColumnLabel",
			        DuplicateAccountDetectionSubscriptionResultXInfo.STATE.getColumnLabel(ctx)));
		...
		{
			DuplicateAccountDetectionSubscriptionResult bean;

			boolean b = true;
			if (j > -1 && j < beans.size())
			{
				bean = (DuplicateAccountDetectionSubscriptionResult) i.next();
				b = true;
			}
			else
			{
				bean = new DuplicateAccountDetectionSubscriptionResult();
				b = false;
			}
			...
			if (mode == EDIT_MODE || mode == CREATE_MODE)
			{
				    ...
				    .getBoolean(com.redknee.framework.xhome.web.Constants.TABLEWEBCONTROL_REORDER_KEY))
				{
					...
					else if (j == 0)
					{
						...
						// "','" +
						// SEPERATOR+"',"+j+","+(j+1)+",new Array('_enabled','subscriptionType','phoneNumber','state'));\" src=\"/images/list/down.gif\"></img>");
					...
					else
					{
						...
						// out.println("<img onclick=\"swapTableLines('"+name +
						// "','" +
						// SEPERATOR+"',"+j+","+(j-1)+",new Array('_enabled','subscriptionType','phoneNumber','state'));\" src=\"/images/list/up.gif\"></img>");
						out.println("</td></tr><tr><td>");
						out.println("<img onclick=\"swapTableLines(this," + j
						    + "," + (j + 1) + ",'" + name + "','"
						    + WebSupport.fieldToId(ctx, name)
			...
			if (subscriptionType_mode != ViewModeEnum.NONE)
			{
				...
				subCtx
				    .put(
				        PROPERTY,
				        DuplicateAccountDetectionSubscriptionResultXInfo.SUBSCRIPTION_TYPE);
				getSubscriptionTypeWebControl()
				    .toWeb(
				        (subscriptionType_mode == ViewModeEnum.READ_ONLY) ? secureCtx
				            : subCtx, out,
				        name + SEPERATOR + j + SEPERATOR + "subscriptionType",
				        Long.valueOf(bean.getSubscriptionType()));
				renderer.TDEnd(ctx,out);
			...
			if (phoneNumber_mode != ViewModeEnum.NONE)
			{
				...
				subCtx
				    .put(
				        PROPERTY,
				        DuplicateAccountDetectionSubscriptionResultXInfo.PHONE_NUMBER);
				getPhoneNumberWebControl().toWeb(
				    (phoneNumber_mode == ViewModeEnum.READ_ONLY) ? secureCtx
				        : subCtx, out,
				    name + SEPERATOR + j + SEPERATOR + "phoneNumber",
				    bean.getPhoneNumber());
				renderer.TDEnd(ctx,out);
			...
			if (state_mode != ViewModeEnum.NONE)
			{
				...
				// bean property they are dealing with.
				subCtx.put(PROPERTY,
				    DuplicateAccountDetectionSubscriptionResultXInfo.STATE);
				getStateWebControl()
				    .toWeb(
				        (state_mode == ViewModeEnum.READ_ONLY) ? secureCtx
				            : subCtx, out,
				        name + SEPERATOR + j + SEPERATOR + "state",
				        bean.getState());
				renderer.TDEnd(ctx,out);
			...
			if (show_actions)
			{
				...
				((WebActionSupport) ctx.get(WebActionSupport.class))
				    .writeLinks(subCtx, beanActions, out, bean,
				        DuplicateAccountDetectionSubscriptionResultIdentitySupport
				            .instance());
			}
