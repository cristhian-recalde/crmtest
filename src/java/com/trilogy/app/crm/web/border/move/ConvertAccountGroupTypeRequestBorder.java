...
/**
 * This border generates an ConvertGroupTypeRequestBorder and puts it in the context.
 * It also sets the read/write mode of any fields that need customization.
 *
 * @author Kumaran Sivasubramaniam
 * @since 8.1
 */
public class ConvertAccountGroupTypeRequestBorder extends AbstractMoveRequestBorder
{
    ...
    public void service(Context ctx, HttpServletRequest req, HttpServletResponse res, RequestServicer delegate) throws ServletException, IOException
    {
        ...
        MoveRequest request = MoveRequestSupport.getMoveRequest(sCtx, account, ConvertAccountGroupTypeRequest.class);
        sCtx.put(MoveRequest.class,request);
        sCtx.put(ConvertAccountGroupTypeRequest.CONVERT_ACCOUNT_GROUP_TYPE, Boolean.TRUE);
        if (request instanceof ConvertAccountGroupTypeRequest)
        {   
            setViewModes(sCtx, account, (ConvertAccountGroupTypeRequest)request);
        }
