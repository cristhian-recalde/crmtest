 ...
 * The reason that the BulkLoadSubcriberCSVHome shouldn't use the xgenerated BulkLoadSubscriberCSVHome
 * (com.redknee.app.crm.customer.hummingbird.bean.account.BulkLoadSubscriberCSVHome) since it
 ...
 * BulkLoadSubscriberCSVHome(Context ctx, String filename, char seperator)
 *
 ...
 */
public class CustomizedBulkLoadSubscriberCSVHome extends AbstractHome implements Constants
{
    ...
    public CustomizedBulkLoadSubscriberCSVHome(final Context ctx, final CSVSupport csvSupport, final FileChannel fileChannel, 
            final String filename) throws HomeException
    {
        this(ctx, csvSupport, filename, fileChannel, DEFAULT_SEPERATOR);
    }

    public CustomizedBulkLoadSubscriberCSVHome(final Context ctx, final CSVSupport csvSupport,
    ...
    public CustomizedBulkLoadSubscriberCSVHome(final Context ctx, final String filename, final FileChannel fileChannel, final char seperator)
        throws HomeException
    {
        this(ctx, BulkLoadSubscriberCSVSupport.instance(),
                filename, fileChannel, seperator);
    }

    public CustomizedBulkLoadSubscriberCSVHome(final CSVSupport csvSupport, final String filename, final FileChannel fileChannel)
        throws HomeException
    {
    ...
    public Visitor forEach(final Context ctx, final Visitor visitor, final Object where) throws HomeException
    {
        ...
        while (i.hasNext())
        {
            ...
            catch (Throwable th)
            {
                ...
                pw.println("Subscriber Load Error: Failure parsing the following line: ");
                pw.print("  ");
                pw.println(line);
