...
/**
 * Represents the Free Call Time Template Creation event record (???) described
 * in the E-Care FS.
 *
 * @author Lily Zou
 * @date   Dec 10, 2004
 */
public final
class ServiceActivationTemplateCreationEventRecord
    implements EventRecord
{
    /**
     * Creates a new ServiceActivationTemplateCreationEventRecord for the given
     * template.
     *
     * @param template The ServiceActivation Template for which the ER will be
     * created.
     *
     * @exception IllegalArgumentException Thrown if the given template is null.
     */
    public ServiceActivationTemplateCreationEventRecord(final  ServiceActivationTemplate template)
    {
        if (template == null)
        {
    ...
    public void generate(final Context context)
    {
        new ERLogMsg(
            ...
            new String[]
            {
                ...
                String.valueOf(template_.getInitialBalance()),
                "0",
                String.valueOf(template_.getMaxBalance()),
                String.valueOf(template_.getMaxRecharge()),
                String.valueOf(template_.getPricePlan()),
                String.valueOf(template_.getReactivationFee())
            }).log(context);
    }
     ...
     */
    private static final String TITLE = "Subscriber Creation Template Creation Event";
