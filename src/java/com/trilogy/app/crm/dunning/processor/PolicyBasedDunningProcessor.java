...
import com.trilogy.app.crm.dunning.SubscriberDunningLevelForecasting;
 ...
 * since 10.1.4
 */
public class PolicyBasedDunningProcessor extends AbstractDunningProcessor
{
    public PolicyBasedDunningProcessor(Date runningDate)
    {
        runningDate_ = runningDate;
    }
    
    @Override
    public void processAccount(Context context, Account account) throws DunningProcessException {
            ...
            && !DunningProcessHelper.isAccountTemporaryDunningExempt(subCtx, account,getRunningDate()))
        {
            ...
            if(!nextLevel.isLevelZero() )
            {
                ...
                subscribersNextLevel = forecastSubscribersLevelAndAddActions(subCtx, account, dunningPolicy, actionHolder, currency);
                
            }else if(nextLevel.isLevelZero() && account.getState() != AccountStateEnum.ACTIVE)
            {
            ...
            updateAccountAndSubscriberNextLevels(subCtx, account, nextLevel, subscribersNextLevel);
          }
        }
        finally
        {
    ...
    private void updateDunningWaiverStatus(Context subCtx, Account account, boolean flag, DunningLevel nextLevel) throws DunningProcessException {
    	
    	
    	if(subCtx.has(DunningConstants.DUNNING_IS_OTG_APPLIED) && subCtx.getBoolean(DunningConstants.DUNNING_IS_OTG_APPLIED))
        {
        	...
        	{
        		try {
					Collection<DunningWaiver> dw= home.select(subCtx, condition);
        	...
        	try 
        	{
    			...
    			if(dw!=null)
    			{
    				...
    				else if(nextLevel.getId() < account.getLastDunningLevel())
    				{
    ...
    private boolean checkForTemporaryDunningWavier(Context context, Account account) throws DunningProcessException 
    {
		...
		where.add(new NEQ(DunningWaiverXInfo.STATUS, OTGDunningStatusEnum.EXPIRED));
	...
	private void setSubscriberLevelsToZero(Context context,DunningLevel nextLevel,
			Account account, Map<String, DunningLevel> subscribersNextLevel) {

    	subscribersNextLevel = new HashMap<String, DunningLevel>();
        try{
	            Collection<Subscriber> allSubscribers = account.getSubscribers(context);
        ...
        {
        	LogSupport.minor(context, this, "Problem setting level zero in subscriber level Map", e);
        }
              
		
	}

	private void addAccountActions(DunningLevel nextLevel,Account account,DunningActionHolder actionHolder)
    {
    ...
    @Override
    public DunningReportRecord generateReportRecord(Context context,Account account) throws DunningProcessException {
        DunningPolicy dunningPolicy = getDunningPolicyOfAccount(context, account);
        ...
        final PMLogMsg pmLogMsg = new PMLogMsg("Generate Report Record", "PolicyBasedDunningProcessor", details);
        try
        {
            
            DunningReportRecord record = null;
            if (dunningPolicy.isDunningExempt(context) || 
                    DunningProcessHelper.isAccountTemporaryDunningExempt(context, account,getRunningDate()))
            ...
            else
            {
                ...
                record.setCurrentLevel(getLevelInfoId(context, dunningPolicy, account.getLastDunningLevel()));
                //for TT ITSC-4196
                ...
                record.setAgedDebt(DunningReportGenerationHelper.createDunningReportRecordAgedDebtList(context, account.getSpid()));
                ...
                DunningLevel nextLevel = forecastable.calculateForecastedLevel(context, account, invoicedAgedDebts, currency, record.getAgedDebt(), false,
                        dunningAmount, dunnedAgedDebt,getRunningDate(),dunningPolicy);
                ...
                if(!nextLevel.isLevelZero() && nextLevel.isAfterLevel(account.getLastDunningLevel()))
                {
                    ...
                    nextLevel = forecastable.calculateForecastedLevel(context, account, agedDebts, currency, record.getAgedDebt(), true,
                            dunningAmount, dunnedAgedDebt, getRunningDate(), dunningPolicy);
                    if(!nextLevel.isLevelZero() && nextLevel.isAfterLevel(account.getLastDunningLevel())){
    ...
    private Map<String,DunningLevel> forecastSubscribersLevelAndAddActions(Context context, Account account, DunningPolicy policy,
            DunningActionHolder actionsHolder, Currency currency) throws DunningProcessException
    {
        Map<String,DunningLevel> nextLevels = new HashMap<String, DunningLevel>();
        try{
            Collection<Subscriber> allSubscribers = account.getSubscribers(context);
            for(Subscriber sub : allSubscribers)
            {
                
                long subscriptionType = sub.getSubscriptionType();
                
                              
                List<SubscriberAgedDebt> subAgedDebt = sub.getSubscriberAgedDebts(context, getSubscriberAgedDebts(context, sub, policy));
                
                Forecastable forecastable = new SubscriberDunningLevelForecasting();
                DunningLevel subNextLevel = forecastable.calculateForecastedLevel(context, sub, subAgedDebt, currency, true, getRunningDate(), policy);
                
                if (subNextLevel != null && !subNextLevel.isLevelZero() && LogSupport.isDebugEnabled(context))
                {
                    StringBuilder sb = new StringBuilder();
                    sb.append("Subscriber :"); sb.append(sub.getMsisdn());
                    sb.append(", Forecasted Level is not L0. DunningLevel :" + subNextLevel);
                    sb.append(" Running Date : "); sb.append(CoreERLogger.formatERDateDayOnly(runningDate_));
                    LogSupport.debug(context, this, sb.toString());
                }
                ...
                nextLevels.put(sub.getId(), subNextLevel);
                if(!subNextLevel.isLevelZero())
                {
                    addSubscriptionAction(subNextLevel,subscriptionType,sub,actionsHolder);
                }else if(subNextLevel.isLevelZero() && sub.getState() != SubscriberStateEnum.ACTIVE){
                    addSubscriptionLevelZeroAction(subNextLevel,subscriptionType,sub,actionsHolder);
                }
    ...
    private void addSubscriptionLevelZeroAction(DunningLevel subNextLevel,
            long subscriptionType, Subscriber sub,
            DunningActionHolder actionsHolder) {
        SubscriberStateChange subAction = new SubscriberStateChange();
        subAction.setNewState(SubscriberStateEnum.ACTIVE);
        List<DunningAction> action = new ArrayList<DunningAction>();
        action.add(subAction);
        actionsHolder.addSubscriberDunningAction(sub.getId(),action);
    }
    ...
    private void populateDRRAgedDebt(Context context,Account account,DunningReportRecord record,List<AgedDebt> agedDebts,Date runningDate)
    {
        List<Date> agedDebtDates = DunningReportGenerationHelper.createAgedDebtDatesList(context, account.getSpid(),runningDate);
        if(agedDebts != null)
        {
    ...
    private void updateAccountAndSubscriberNextLevels(Context ctx,Account account,DunningLevel acccountNextLevel,
            Map<String,DunningLevel> subscribersNextLevel)
    {
        ...
        {
            LogSupport.minor(ctx, this, "Problem updating Account next Dunning Level", e);
        }
        
        try{
            if(subscribersNextLevel != null)
            {
                Collection<Subscriber> allSubscribers = account.getSubscribers(ctx);
                
                for(Subscriber sub : allSubscribers)
                {
                    
                    if(subscribersNextLevel.get(sub.getId()) != null)
                    {
                        DunningLevel nextLevel = subscribersNextLevel.get(sub.getId());
                        if(nextLevel.getId() != sub.getLastDunningLevel())
                        {
	                    	sub.setLastDunningLevel(nextLevel.getId());
	                        HomeSupportHelper.get(ctx).storeBean(ctx, sub);
                        }
                    }else{
                        LogSupport.minor(ctx, this, "Problem updating Subsriber next Dunning Level due to null for "+sub.getId() );
                    }
                }
            }
        }catch(HomeException e)
        {
            LogSupport.minor(ctx, this, "Problem updating Subsriber next Dunning Level", e);
        }
        
    }
    
    private int getLevelInfoId(final Context ctx, final DunningPolicy policy,int levelId)
    {
        if(levelId == 0)
        {
            return levelId;
