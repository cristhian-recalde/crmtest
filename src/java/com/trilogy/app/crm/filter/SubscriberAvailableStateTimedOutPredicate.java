...
/**
 * A Predicate used to find Subscribers (in Available state) whose availableTimers
 * match that of its service provider.
 *
 * @author jimmy.ng@redknee.com
 */
public class SubscriberAvailableStateTimedOutPredicate
	implements Predicate
{
     ...
     */ 
	public SubscriberAvailableStateTimedOutPredicate(final CRMSpid serviceProvider, final Date expirationDate)
	{
	...
	public boolean f(Context _ctx,final Object obj)
	{
        ...
        if (subscriber.getState() == SubscriberStateEnum.AVAILABLE &&
			...
			/*&&
            subscriber.getAvailableTimer() >= serviceProvider_.getSubAvailableTimer())*/
        {
            return true;
        }
