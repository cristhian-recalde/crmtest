...
public class TransferDisputeAcceptWebAction
    extends SimpleWebAction
{
    public TransferDisputeAcceptWebAction()
    {
        super("acceptDispute", "Accept Dispute");
    }
        ...
        throws AgentException
    {
        ...
        try
        {
            TransferDisputeCmd cmd = new TransferDisputeCmd(Long.valueOf(key), TransferDisputeCmd.ACCEPT_CMD);
            Home h = (Home)ctx.get(TransferDisputeHome.class);

            h.cmd(ctx, cmd);
            printMessage(out, "Successfully accepted dispute");     
        }
        catch(Exception e)
        {
            LogSupport.major(ctx, this, "Unable to accept dispute [" + key + "]", e);
            
            printError(out, "Unable to accept dispute [key=" + key + "] due to Exception:<br>" + e.getMessage());
        }
