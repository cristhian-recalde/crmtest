...
import com.trilogy.dynamics.crm.crmservice._2007.webservices.Rkn_region;
import com.trilogy.dynamics.crm.crmservice.types.Guid;


/**
 * Provides a method to update DCRM with Province information.
 *
 * @author gary.anderson@redknee.com
 */
public class DcrmProvinceSync
    implements HomeChangeListener, DcrmSync
{
    /**
     * The DCRM name of the entity.
     */
    private static final String ENTITY_NAME = "rkn_region";


    /**
     * Useful for identifying this DcrmSync.
     */
    public static final String KEY = "DcrmProvinceSync";


    /**
     * The pipeline key used to get the pipeline from the operating context.
     */
    private static final Class<ProvinceHome> PIPELINE_KEY = ProvinceHome.class;
    ...
    public void updateAll(final Context context)
    {
        final Visitor updateVisitor = new Visitor()
        {
            ...
            {
                updateProvince(context, (Province)obj);
            }
        };
    ...
    public Guid[] getDcrmGuids(final Context context, final BusinessEntity[] businessEntities)
    {
        ...
        {
            guids[index] = ((Rkn_region)businessEntities[index]).getRkn_regionid();
        }
    ...
    public void homeChange(final HomeChangeEvent evt)
    {
        ...
        {
            updateProvince(context, province);
        }
        else if (evt.getOperation() == HomeOperationEnum.CREATE)
        {
            createProvince(context, province);
        }
        else if (LogSupport.isDebugEnabled(context))
        {
            new DebugLogMsg(this, "Unsupported Home Operation " + evt.getOperation() + " for " + province, null).log(context);
        }
    }
     ...
     * @param province The bean for which a GUID is needed.
     * @return The GUID if one exists; null otherwise.
     */
    public Guid getDcrmGuid(final Context context, final Province province)
    {
        final String provinceID = province.getName();
        return getDcrmGuid(context, provinceID);
    }
     ...
     *
     * @param context The operating context.
     * @param provinceID The ID of the bean for which a GUID is needed.
     * @return The GUID if one exists; null otherwise.
     */
    public Guid getDcrmGuid(final Context context, final String provinceID)
    {
        ...
        condition.getValues().addValue(provinceID);
        ...
        final Guid primaryGuid = DcrmSyncSupport.getGuid(context, "rkn_regionid", conditions, this);
     ...
     *
     * @param context The operating context.
     * @param province The bean to create in DCRM.
     */
    private void createProvince(final Context context, final Province province)
    {
        final Rkn_region dcrmProvince = convert(context, province);

        final Guid response = DcrmSyncSupport.create(context, dcrmProvince);

        if (response != null && LogSupport.isDebugEnabled(context))
        {
            new DebugLogMsg(this, "Province Created: " + response.getGuid(), null).log(context);
        }
        else
        {
            new MajorLogMsg(this, "Failed to create Province " + province.getName(), null).log(context);
        }
    }
     ...
     *
     * @param context The operating context.
     * @param province The bean to update in DCRM.
     */
    private void updateProvince(final Context context, final Province province)
    {
        final Rkn_region dcrmProvince = convert(context, province);
        final Guid dcrmGuid = getDcrmGuid(context, province);

        if (dcrmGuid != null)
        {
            final Key key = new Key();
            key.setGuid(dcrmGuid.getGuid());
            dcrmProvince.setRkn_regionid(key);

            DcrmSyncSupport.update(context, dcrmProvince);
        }
        else
        {
            new MajorLogMsg(this, "Failed to look-up corresponding Province: " + province.getName(), null).log(context);
            createProvince(context, province);
        }
    }
     ...
     *
     * @param context The operating context.
     * @param province The bean to convert.
     * @return An instance of a DCRM entity.
     */
    private Rkn_region convert(final Context context, final Province province)
    {
        final Rkn_region dcrmProvince = new Rkn_region();

        // Set the primary key.
        {
            dcrmProvince.setRkn_identifier(province.getName());
        }

        // Set the description.
        {
            dcrmProvince.setRkn_name(province.getName());
        }

        // Set the SPID.
        {
            final Lookup spid = DcrmServiceProviderSync.getDcrmLookup(context, province.getSpid());
            dcrmProvince.setRkn_serviceproviderid(spid);
        }
